...
The [README](../README.md) covers the quick start. This page has everything
else: the allowlist format, the Recovery web panel, Docker details, workstation
hardening, CI integration, YARA rules, the live-sample write-up, and project
layout.
...
[ENTERPRISE_RECOVERY.md](ENTERPRISE_RECOVERY.md).
...
1. **Check what would be cleaned** — read-only, always safe, no confirmation
   ...
   Also lists every distinct file affected, each with its own checkbox, so
   you can exclude a specific false positive from the rewrite below.
2. **Rewrite history** — requires typing `REWRITE HISTORY` into a confirm
   ...
   modified directly, matching the CLI's own safety model. Any file you
   unchecked in the per-file list above is left completely untouched.
...
It supports three sources, picked with a toggle in the form:
...
- **Git URL** — `https://`, `git://`, `ssh://`, or `git@host:path`, plus an
  ...
  correctly) and scans it, showing live clone progress. The clone is
  **kept on disk after scanning** rather than deleted, so Recovery and the
  file viewer have something to work with; it's replaced the next time you
  run a URL scan, not before (this is a single-user local tool, so "keep the
  one most recent clone around" is the whole cleanup policy — there's no TTL
  or multi-scan cache). For a private repo, paste a personal access token
  into the token field; it's sent once for that scan only (never stored,
  never logged, and stripped out of any error message before it reaches the
  browser), and is only accepted for `http(s)` URLs. A failed clone that
  looks auth-related gets a hint to add a token. `file://` URLs are rejected
  here — use the local path option below for anything already on disk.
...
- **GitHub org/user (all repos)** — see below.
...
clones and scans each repo in turn, showing live clone progress per repo.
Unlike a single URL scan, there's no single "most recent clone" slot to
reuse the way one repo has, so every successfully-scanned repo's clone is
instead kept on disk and tracked, listed in a **Temp clones** panel where
you can inspect, reuse, or delete each one (individually or all at once) —
a failed or timed-out clone is still discarded immediately. Reuses the same
...
(keeping its clone, same as the rest of an org scan) and folds the result
into the dashboard in place, rather than requiring a full re-run.
...
[Recovery](#recovering-from-the-web-ui) is available after any of the three
scan modes, for the same reason: clones are no longer deleted right after
scanning.

Binds to `127.0.0.1` only by default. All of this is meaningfully more
exposed than a plain read-only page — the URL and org modes clone whatever
they're given (and keep those clones on disk between scans), the local-path
mode and file viewer read whatever path they're given, and the folder
browser walks the directory tree of whatever machine is running the
server — so don't put this on a shared interface without your own auth in
front of it. Passing a non-loopback `--host` prints a warning to the
console as a reminder.
...
[GITHUB_PROTECTION.md](GITHUB_PROTECTION.md).
...
longer has — see the note on removed detectors in the README), for
